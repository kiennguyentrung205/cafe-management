...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, ApplicationContext applicationContext) throws Exception {
        ...
        http.csrf(Customizer.withDefaults()).authorizeHttpRequests(auth -> auth.requestMatchers("/login**", "/assets/**", "/forgot-password", "/set-password**", "/register", "/home").permitAll().requestMatchers("/table/booking/management").hasRole("CASHIER")
                        ...
                        .requestMatchers("/dashboard/staff/**").hasRole("ADMIN").requestMatchers("/dashboard/vouchers/list").hasRole("ADMIN").requestMatchers("/dashboard/vouchers/create").hasRole("ADMIN").requestMatchers("/dashboard/vouchers/edit/**").hasRole("ADMIN").requestMatchers("/dashboard/vouchers/deleted-list").hasRole("ADMIN")
                        .requestMatchers("/product/list").hasRole("ADMIN").requestMatchers("/dashboard/staff").hasRole("ADMIN")
